...
class Transformer(nn.Module):
    def __init__(self, inp_res=32, dim=256, depth=2, num_heads=4, mlp_ratio=4.0, injection=True):
        ...
        for _ in range(depth):
            self.layers.append(
                Block(dim=dim, num_heads=num_heads, mlp_ratio=mlp_ratio, injection=injection)
            )
...
class Block(nn.Module):
    def __init__(
        ...
        self.encode_query = nn.Conv2d(
            in_channels=dim, out_channels=dim, kernel_size=1, stride=1, padding=0
        )
        self.encode_key = nn.Conv2d(
            in_channels=dim, out_channels=dim, kernel_size=1, stride=1, padding=0
        )

        if self.injection:
            self.encode_query2 = nn.Conv2d(
                in_channels=dim, out_channels=dim, kernel_size=1, stride=1, padding=0
            )
            self.encode_key2 = nn.Conv2d(
                in_channels=dim, out_channels=dim, kernel_size=1, stride=1, padding=0
            )
    ...
    def forward(self, query, key, query_embed=None, key_embed=None):
        ...
        if self.injection:
            ...
            query = query + self.attn(
                query=q, key=k, value=v, query2=q2, key2=k2, use_sigmoid=False
            )
