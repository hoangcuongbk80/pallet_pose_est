...
class ManoLayer(Module):
    ...
    def __init__(
        ...
        # Get hand mean
        hands_mean = (
            np.zeros(hands_components.shape[1]) if flat_hand_mean else smpl_data["hands_mean"]
        )
    ...
    def forward(
        ...
        if self.use_pca or self.joint_rot_mode == "axisang":
            ...
            ), "When not self.use_pca, " "th_pose_coeffs should have 4 dims, got {}".format(
                th_pose_coeffs.dim()
            )
        ...
        if th_betas is None or th_betas.numel() == 1:
            th_v_shaped = (
                torch.matmul(self.th_shapedirs, self.th_betas.transpose(1, 0)).permute(2, 0, 1)
                + self.th_v_template
            )
            ...
            th_v_shaped = (
                torch.matmul(self.th_shapedirs, th_betas.transpose(1, 0)).permute(2, 0, 1)
                + self.th_v_template
            )
        ...
        th_v_posed = th_v_shaped + torch.matmul(
            self.th_posedirs, th_pose_map.transpose(0, 1)
        ).permute(2, 0, 1)
        ...
        root_trans_flt = (
            root_trans.unsqueeze(1).repeat(1, 5, 1, 1).view(root_trans.shape[0] * 5, 4, 4)
        )
        ...
        th_rest_shape_h = torch.cat(
            [
                ...
                torch.ones(
                    (batch_size, 1, th_v_posed.shape[1]), dtype=th_T.dtype, device=th_T.device
                ),
            ],
            1,
        )
